...
当检测到频道题材不在已验证范围内（如民国谍战、赛博朋克、丧尸、电竞等）时，执行以下分层处理：

### 数据层（不降级）
- 四象限归类、瓶颈定位、订阅转化、发布节奏等**数据层诊断照常运行**，不受题材影响
- CTR/AVD/留存是物理指标，题材无关，结论置信不降

### 内容层（显式降置信）
- 封面/标题/标签建议必须显式声明：
  > "该题材未入库已验证数据，以下为通用模板卡 + 最近邻题材（注明借用哪个）的迁移推断，置信有限。"
- 禁止假装该题材已有验证数据
- 封面符号：从通用模板卡出发，借用最相似的已验证题材符号（如"谍战"借"复仇/打脸"的冷色调+对峙构图），明确标注借源
- 标题句式：用通用骨架+反转词，题材行话由用户补充

### 自动建档
- 在 `knowledge/pending.md` 自动创建该题材条目，格式：
  `日期 | 新题材:{名称} | 假设符号:{推测的场景/道具/色彩} | 假设骨架:{最可能的骨架类型} | 待CTR/封面数据验证 | 状态:待验证`
- 该题材后续视频的CTR数据回流后，有效假设进 `validated.md`，失效进 `lessons.md`
...
**触发规则**：
- 点赞率 < 0.5% → **critical**（行业基准1.5-2%）：动作：标题前10字上冲突钩子；封面用中景/中近景展示冲突场景和情绪表情；前5秒口播引导点赞。
...
### 步骤 9：封面协同诊断（需封面数据）
**看什么**：构图站位分、情绪表达分、关键道具分、文字标签分。
**触发规则**（各项<阈值）：
- 构图站位 < 6 → **major**：中景/中近景为主，展示人物姿态和冲突关系（爆款实测肤色占比仅~10%，不必追求面部60%+）。
